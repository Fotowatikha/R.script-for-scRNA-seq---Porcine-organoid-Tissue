...
output: tiss.vs.org_notebook
---

# In the chuck below, we load in all the essential libraaries for the data analysis. 
```{r Load Libraries, echo=FALSE, results=FALSE}
library(dplyr)
library(dbplyr)
library(Seurat)
library(SeuratDisk)
...
######################################### Porcine Ileum Tissue vs Organoid (Integration) #########################################
...
##### Alternative, because we cannot open and save Seurat_v5 files. an update is needed for LoadH5Seurat package ######

#### NEW Seurat v5 Method ####
# Let make second packups
sobj.org <- sobj1
sobj.org.noSecProg <- sobj1.1
sobj.tiss.all <- sobj3
sobj.tiss.epit <- sobj2

sobj <- sobj.tiss.epit

# Merge the Seurat objects that we will integrate later on
org.tiss.combined <- merge(x= sobj.org, y=sobj.tiss.epit)

gc(reset = TRUE) # clean memory

#### OLD Seurat v4 Method ####
# Find Seurat file names
#sob_names <- list.files(parent_directory, pattern=NULL, all.files=FALSE, full.names=FALSE)
#sob_names

# Opening the Seurat objects and putting them in a vector
#sobj_list <- list() # make empty list to append Seurat Objects in the loop below
#for (i in 1:length(x = sob_names)) {  # we loop in range of sob_names, because that determine the number of replicates we have
#  name <- paste('', sob_names[i], sep='') # name the objects in list
#  sobj_list[name] <- LoadH5Seurat(paste(sob_names[i])) # append the Seurat objects in a list of Seurat Objects
#}
```
...
# One aprouch to calculate the highly variable genes is by calculation the stdv/mean-count. So genes with low expression will have large variance when compared to highyl expressed genes.
...
#  sobj <- FindVariableFeatures(sobj, selection.method = "vst", nfeatures = 2000) #### AOTOMATE THE NUMBER OF VARIABLES
#  return(sobj)
#}
...
#highly_variable_feautures <- SelectIntegrationFeatures(object.list = sobj_list, nfeatures = 2000)
```
...
# Scaling is done on genes that are interesting (by this we mean the highly variable genes, becaue these highy variable genes needs to scaled)
...
- The sclaing is done as follows:
...
# The reason that we do a PCA is because we want to potential din the optimal number of dimensions that will be used for the integration anchoring in RPCA or CCA. 
...
#  co1 # list PC
#  co2 <- sort(which((pc.stdev.percentage[1:length(pc.stdev.percentage) - 1] - pc.stdev.percentage[2:length(pc.stdev.percentage)]) > 0.05), decreasing = T)[1]  # find last PC where change in percent variation is more than 0.5%
#  co2 # list PC
...
# Since we have already sclaled our data, we od not have to do it again, see sclaing method in previous codes
...
# find anchors CCA
#find.anchors <- FindIntegrationAnchors(object.list = sobj_list, anchor.features = highly_variable_feautures, reduction = "cca", dims = 1:30)
# Now we creates the 'integrated' data assay
#org.tiss.combined <- IntegrateData(anchorset = find.anchors, dims = 1:30)
```



#################### Standard Workflow on Integrated dataset ####################
...
#### OLD Seurat v4 Method ####
# Scale the data
#####org.tiss.combined <- NormalizeData(object = org.tiss.combined, assay = "RNA") #### DO not run this code, this is only for test
...
```{r final cluster, fig.height=5, fig.width=8, echo=FALSE} 
#### NEW Seurat v5 Method ####
...
#### OLD Seurat v4 Method ####
# subset of Goblet, Panneth/goblet early, and goblet organoid
#DimPlot(org.tiss.combined[,org.tiss.combined$cellID == "Goblet Cells" | org.tiss.combined$cellID == "Panneth Cells" | org.tiss.combined$cellID == "Secretory Cells (Goblet/Enteroendocrine)" | org.tiss.combined$cellID == "Enteroendocrine Cells"], group.by = 'cellID', #pt.size = 1.2, cols = c('Goblet Cells' = 'pink', 'Panneth Cells' = 'darkmagenta', 'Secretory Cells (Goblet/Enteroendocrine)' = 'magenta', "Enteroendocrine Cells" = "lightblue"))

#DimPlot(org.tiss.combined[,org.tiss.combined$cellID == "Goblet Cells" | org.tiss.combined$cellID == "Paneth Cells" | org.tiss.combined$cellID == "Secretory Cells (Goblet/Enteroendocrine/Paneth)" | org.tiss.combined$cellID == "Enteroendocrine Cells" | org.tiss.combined$cellID == "Goblet Cells *" | org.tiss.combined$cellID == "Paneth Cells *" | org.tiss.combined$cellID == "Enteroendocrine Cells *"], group.by = 'cellID', pt.size = 1.2, cols = c('Goblet Cells' = 'pink', 'Paneth Cells' = 'darkmagenta', 'Secretory Cells (Goblet/Enteroendocrine/Paneth)' = 'magenta', "Enteroendocrine Cells" = "gold2", 'Goblet Cells *' = 'pink4', 'Paneth Cells *' = 'red3', "Enteroendocrine Cells *" = "lightblue4"))

# highlighting cells of interest
#DimPlot(object = org.tiss.combined[,org.tiss.combined$cellID == "Goblet Cells" | org.tiss.combined$cellID == "Paneth Cells" | org.tiss.combined$cellID == "Secretory Cells (Goblet/Enteroendocrine/Paneth)" | org.tiss.combined$cellID == "Enteroendocrine Cells" | org.tiss.combined$cellID == "Goblet Cells *" | org.tiss.combined$cellID == "Paneth Cells *" | org.tiss.combined$cellID == "Enteroendocrine Cells *"], cells.highlight = c("ACGTCCTAGCACCTGC-1_1", "CTTCAATCAGGAGACT-1_2", "TCACGGGAGTCCCAGC-1_2", "TCGTAGAAGACTTCAC-1_2"), cols.highlight = "red", cols = "gray", order = TRUE)
```
...
####### TEST ####### SCLAE.DATA REMOVES GENES (SOX9), FIX THIS PROBLEM!!!!!!!!
#DoHeatmap(subset(pic1[,pic1$cellID == "Prolif Prog Cells" | pic1$cellID == "Prolif Prog Cells *" | pic1$cellID == "Stem Cells *"], downsample = 200), features = stem.prog_markers, label = FALSE, group.by = 'cellID') + scale_fill_gradientn(colors = c("darkturquoise", "grey90", "red")) #+ NoLegend()
...
# Violin plot test
#VlnPlot(org.tiss.combined[,org.tiss.combined$cellID == "Prolif Prog Cells" | org.tiss.combined$cellID == "Stem cells" | org.tiss.combined$cellID == "Semi-Prolif Stem/Prog Cells"], features = stem.prog_markers, split.by = "cell.origin", group.by = "cellID", pt.size = 0, combine = FALSE, log = T)
#VlnPlot(org.tiss.combined, features = stem.prog_markers, log = TRUE)
 
# correctly scale data (re-run this every time for heatmap)   
#test <- ScaleData(org.tiss.combined, verbose = FALSE, assay="RNA")   
```
...
### ONLY RUN THE THREE LINES BELOW, IF DATA IS NOT SCALED AND NORMALIZED) FOR INEGRATED DATA ONLY###
#Idents(org.tiss.combined) <- "cellID" # Set the idents of the metadata to the one of interest (in this case our cellIDs)
#org.tiss.combined <- NormalizeData(object = org.tiss.combined, normalization.method = "LogNormalize", scale.factor = 10000, assay = "RNA")
#org.tiss.combined <- ScaleData(org.tiss.combined, verbose = FALSE, assay="RNA") 

### DO CORRELATION OF INTEGRATED DATA ###
### (THIS IS THE VERSION WE WANT TO RUN!!!!! ####
...
highly_variable_feautures <- VariableFeatures(org.tiss.combined)
...
```{r correlation of average expression, echo=FALSE,  fig.height=6, fig.width=8}
gc(reset = TRUE) # clean memory
